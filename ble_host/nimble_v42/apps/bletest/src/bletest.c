...
//#define MYNEWT_VAL_BLETEST_ROLE BLETEST_ROLE_SCANNER


#if (MYNEWT_VAL_BLETEST_ROLE) == BLETEST_ROLE_ADVERTISER
#define BLETEST_CFG_ROLE                BLETEST_ROLE_ADVERTISER
#endif
#if (MYNEWT_VAL_BLETEST_ROLE) == BLETEST_ROLE_SCANNER
#define BLETEST_CFG_ROLE                BLETEST_ROLE_SCANNER
#endif
#if (MYNEWT_VAL_BLETEST_ROLE) == BLETEST_ROLE_INITIATOR
...
#define BLETEST_CFG_ADV_FILT_POLICY     (BLE_HCI_ADV_FILT_BOTH)
...
#define BLETEST_CFG_SCAN_FILT_POLICY    (BLE_HCI_SCAN_FILT_USE_WL_INITA)
...
#define BLETEST_CFG_INIT_FILTER_POLICY  (BLE_HCI_CONN_FILT_USE_WL)
...
#if ((MYNEWT_VAL_BLE_LL_CFG_FEAT_LL_PRIVACY) == 1)
...
#if ((MYNEWT_VAL_BLE_LL_CFG_FEAT_LE_ENCRYPTION) == 1)
...
#if (MYNEWT_VAL_BLE_ANDROID_MULTI_ADV_SUPPORT)
void
bletest_init_adv_instances(void)
{
    ...
    inst_allowed = (MYNEWT_VAL_BLE_MULTI_ADV_INSTANCES);
...
bletest_send_packet(uint16_t handle)
{
    ...
    if (om) {
        ...
        pktlen = atbm_os_random() % (BLETEST_MAX_PKT_SIZE + 1);
#else
        pktlen = BLETEST_PKT_SIZE;
#endif
#endif
...
#if ((MYNEWT_VAL_BLE_LL_CFG_FEAT_DATA_LEN_EXT) == 1)
...
#if BLETEST_CFG_ROLE == BLETEST_ROLE_ADVERTISER
	printf("BLETEST_ROLE_ADVERTISER\n");
    g_dev_addr[0] = 0x00;
    g_dev_addr[1] = 0x00;
    g_dev_addr[2] = 0x00;
    g_dev_addr[3] = 0x88;
...
#if BLETEST_CFG_ROLE == BLETEST_ROLE_SCANNER
	printf("BLETEST_ROLE_SCANNER\n");
#else
	printf("BLETEST_ROLE_INITIATOR\n");
#endif
    g_dev_addr[0] = 0x00;
    g_dev_addr[1] = 0x00;
    g_dev_addr[2] = 0x00;
