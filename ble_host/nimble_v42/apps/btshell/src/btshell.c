...
#define BTSHELL_COC_BUF_COUNT         (3 * (MYNEWT_VAL_BLE_L2CAP_COC_MAX_NUM))

#define INT_TO_PTR(x)     (void *)(x)
#define PTR_TO_INT(x)     (int) (x)
#endif

struct btshell_conn btshell_conns[(MYNEWT_VAL_BLE_MAX_CONNECTIONS)];
...
static os_membuf_t btshell_coc_conn_mem[
    OS_MEMPOOL_SIZE((MYNEWT_VAL_BLE_L2CAP_COC_MAX_NUM),
...
SRAM_CODE static struct btshell_conn *
...
SRAM_CODE static struct btshell_svc *
...
SRAM_CODE static struct btshell_chr *
...
SRAM_CODE static struct btshell_dsc *
...
btshell_conn_add(struct ble_gap_conn_desc *desc)
{
    ...
    assert(btshell_num_conns < (MYNEWT_VAL_BLE_MAX_CONNECTIONS));
...
btshell_conn_delete_idx(int idx)
{
     ...
     */
#if (MYNEWT_VAL_BLE_MAX_CONNECTIONS) > 1
...
extern short g_scan_show;
extern int g_scan_name_cnt;
extern uint8_t g_scan_name_table[64][64];
extern int g_show_addr;
uint8_t g_scan_addr_table[64][6];
extern int g_scan_addr_cnt;

SRAM_CODE static void
btshell_show_scan_addr(uint8_t* addr, uint8_t addr_len)
{
    int i;
    int find = 0;
    ...
    if(0 == find)
    {
	memcpy(g_scan_addr_table[g_scan_addr_cnt], addr, 6);
	iot_printf("===> advertise addr :");
	print_addr(addr);
	iot_printf("\n");
	if(g_scan_addr_cnt < 63)
	     g_scan_addr_cnt++;
...
btshell_decode_adv_data(uint8_t* adv_data, uint8_t adv_data_len, void* arg)
{
    ...
    if (scan_opts) {
        adv_data_len = atbm_min(adv_data_len, scan_opts->limit);
    }

    if (g_scan_show) {
    	// iot_printf(" length_data=%d data=", adv_data_len);
        // print_bytes(adv_data, adv_data_len);
    	iot_printf(" fields:\n");
    }
    ble_hs_adv_parse_fields(&fields, adv_data, adv_data_len);
    if (g_scan_show == 0) {
        for (i = 0;i < g_scan_name_cnt;i++) {
            if (memcmp(g_scan_name_table[i], fields.name, fields.name_len) == 0) {
                b_find = 1;
            }
        }
        if (b_find == 0) {
            memcpy(g_scan_name_table[g_scan_name_cnt], fields.name, fields.name_len);
            iot_printf("===> fields:\n");
            btshell_print_adv_fields(&fields);
            iot_printf("\n");
            if (g_scan_name_cnt < 63) {
                g_scan_name_cnt++;
            }
            else {
                //all clear ,just for test
                g_scan_name_cnt = 0;
            }
        }
    }
...
extern int scan_show_name_valid;
extern u8 scan_show_name[128];
    ...
    case BLE_GAP_EVENT_DISC:
		if(scan_show_name_valid){
			...
			if(fields.name && fields.name_len){
				if(0 == memcmp(scan_show_name, fields.name, fields.name_len)){
            ...
            if (g_scan_show) {
                iot_printf("rx adv; type=%d rssi=%d "
	                       "addr_type=%d addr=", event->disc.event_type,
	                       event->disc.rssi, event->disc.addr.type);
	        print_addr(event->disc.addr.val);
            }
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) {
            ...
            if(event->passkey.params.action == BLE_SM_IOACT_INPUT){
				...
				//pkey.passkey =123456;//PTS需要输入123456来通过验证。
...
#if (MYNEWT_VAL_BLE_L2CAP_COC_MAX_NUM) != 0
...
{
    iot_printf("LE CoC SDU received, chan: 0x%08x, data len %d\n",
                           ...
                           struct ble_l2cap_chan *chan)
{
    ...
    iot_printf("LE CoC accepting, chan: 0x%08x, peer_mtu %d\n",
...
btshell_l2cap_event(struct ble_l2cap_event *event, void *arg)
{
    ...
    switch(event->type) {
        case BLE_L2CAP_EVENT_COC_CONNECTED:
            ...
            iot_printf("LE COC connected, conn: %d, chan: 0x%08x, scid: 0x%04x, "
        ...
        case BLE_L2CAP_EVENT_COC_DISCONNECTED:
            iot_printf("LE CoC disconnected, chan: 0x%08x\n",
