...
#define  PTS_CHR_READ_MTU_TEST           0x0020
...
static uint8_t gatt_svr_pts_static_ptsmtu_val[256];
...
static const struct ble_gatt_svc_def gatt_svr_svcs[] = {
    {
        ...
        .characteristics = (struct ble_gatt_chr_def[]) { {
                ...
                .uuid = PTS_UUID_DECLARE(PTS_CHR_READ_MTU_TEST),
                .access_cb = gatt_svr_access_test,
                     ...
                     void *arg)
{
    ...
    assert(uuid16 != 0);
    iot_printf("PTS start gatt handle:%d,UUID:%d\n",attr_handle,uuid16);
    switch (uuid16) {
    case PTS_CHR_READ:
        assert(ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR);
        ...
        return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
    case PTS_CHR_READ_MTU_TEST:
        assert(ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR);
        rc = os_mbuf_append(ctxt->om, &gatt_svr_pts_static_ptsmtu_val,
                            sizeof gatt_svr_pts_static_ptsmtu_val);
        return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
    case PTS_CHR_WRITE:
                          ...
                          void *arg)
{
    ...
    uuid16 = extract_uuid16_from_pts_uuid128(ctxt->chr->uuid);
    assert(uuid16 != 0);
    iot_printf("PTS start long gatt handle:%d,UUID:%d\n",attr_handle,uuid16);
    switch (uuid16) {
