...
static int bt_mesh_calc_dh_key(void)
{
     ...
     * X and Y halves are swapped independently.
     */
    sys_memcpy_swap(&buf[0], &link.conf_inputs[17], 32);
    sys_memcpy_swap(&buf[32], &link.conf_inputs[49], 32);

    BT_DBG("private key = %s", bt_hex(bt_mesh_private_key, 32));

    if(ble_sm_alg_gen_dhkey(&buf[0], &buf[32], bt_mesh_private_key, dhkey)){
		BT_ERR("Failed to generate DHKey");
		prov_send_fail_msg(PROV_ERR_UNEXP_ERR);
		//close_link(PROV_ERR_UNEXP_ERR, CLOSE_REASON_FAILED);
		return -1;
	...
	printf("bt_mesh_calc_dh_key +++++++++++++++++\n");
    prov_dh_key_cb(dhkey);
...
static int bt_mesh_prov_inited = false;
...
int bt_mesh_prov_init(const struct bt_mesh_prov *prov_info)
{
	...
	bt_mesh_prov_inited = true;

	return 0;
}

void bt_mesh_prov_deinit(void)
{
	if(!bt_mesh_prov_inited){
		return;
	}
