		    ...
		    const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	BT_INFO("src 0x%04x dst 0x%04x app_idx 0x%04x aszmic %u sdu_len %u",
		     ...
		     enum bt_mesh_friend_pdu_type *pdu_type, u64_t *seq_auth, u8_t *seg_count)
{
	...
	*seq_auth = SEQ_AUTH(BT_MESH_NET_IVI_RX(net_rx),
			     (net_rx->seq - ((((net_rx->seq & MESH_BIT_MASK(14)) - seq_zero)) & MESH_BIT_MASK(13))));
...
found_rx:
	...
	if (seg_o == seg_n) {
		...
		if (rx->buf->om_len > MYNEWT_VAL(BLE_MESH_RX_SDU_MAX)) {
			BT_ERR("Too large SDU len, len:%d", rx->buf->om_len);
			send_ack(net_rx->sub, net_rx->ctx.recv_dst,
				 net_rx->ctx.addr, net_rx->ctx.send_ttl,
				 seq_auth, 0, rx->obo);
			seg_rx_reset(rx, true);
...
int bt_mesh_trans_recv(struct os_mbuf *buf, struct bt_mesh_net_rx *rx)
{
	...
	BT_DBG("src 0x%04x dst 0x%04x ttl 0x%02X ctl 0x%02X seq 0x%08x friend_match %d",
	       rx->ctx.addr, rx->ctx.recv_dst, rx->ctx.recv_ttl, rx->ctl,
