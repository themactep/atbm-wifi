...
static void friend_clear(struct bt_mesh_friend *frnd)
{
	...
	while (!net_buf_slist_is_empty(&frnd->queue)) {
		struct os_mbuf *buf;
		buf = (void *)net_buf_slist_get(&frnd->queue);
	...
	for (i = 0; i < ARRAY_SIZE(frnd->seg); i++) {
		...
		while (!net_buf_slist_is_empty(&seg->queue)) {
			struct os_mbuf *buf;
			buf = (void *)net_buf_slist_get(&seg->queue);
			BT_MESH_ADV(buf)->flags &= ~NET_BUF_FRAGS;
			net_buf_unref(buf);
		}
...
static struct bt_mesh_friend_seg *get_seg(struct bt_mesh_friend *frnd,
					  u16_t src, u64_t *seq_auth, u8_t seg_count)
{
...
static void enqueue_friend_pdu(struct bt_mesh_friend *frnd,
			       ...
			       struct os_mbuf *buf, u8_t seg_count)
{
...
static void friend_timeout(struct ble_npl_event *work)
{
	...
	BT_INFO("Sending buf %p from Friend Queue of LPN 0x%04x, queue_size:%d",
...
static void friend_lpn_enqueue_rx(struct bt_mesh_friend *frnd,
				  ...
				  enum bt_mesh_friend_pdu_type type,
				  u64_t *seq_auth, u8_t seg_count, struct os_mbuf *sbuf)
{
	struct friend_pdu_info info;
	struct os_mbuf *buf;
	...
	enqueue_friend_pdu(frnd, type, buf, seg_count);

	BT_INFO("Queued message for LPN 0x%04x, queue_size %u",
