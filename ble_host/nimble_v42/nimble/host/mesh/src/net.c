...
#if MYNEWT_VAL(BLE_MESH_PROVISIONER)
	.p_sub = {
		[0 ... (MYNEWT_VAL(BLE_MESH_PROVISIONER_SUBNET_COUNT) - 1)] = {
			.net_idx = BT_MESH_KEY_UNUSED,
		}
	},
#endif
		     ...
		     const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	/*support proxy client net PDU*/
    if (MYNEWT_VAL(BLE_MESH_GATT_PROXY_CLIENT) &&
            tx->ctx->send_ttl != 1U) {
        if (bt_mesh_proxy_client_relay(buf, tx->ctx->addr)) {
			      ...
			      struct bt_mesh_net_rx *rx)
{
	...
	BT_DBG("net_relay TTL %u CTL %u dst 0x%04x", rx->ctx.recv_ttl, rx->ctl,
	       rx->ctx.recv_dst);
...
void bt_mesh_net_deinit(void)
{
	...
	k_work_free(&bt_mesh.local_work);
}
