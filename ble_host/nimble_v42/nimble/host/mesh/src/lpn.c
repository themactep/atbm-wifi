...
static inline void group_zero(atomic_t *target)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		mesh_atomic_set(&target[i], 0);
	}
#else
	mesh_atomic_set(target, 0);
#endif
}
...
static inline void group_set(atomic_t *target, atomic_t *source)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		mesh_atomic_or(&target[i], mesh_atomic_get(&source[i]));
	}
#else
	mesh_atomic_or(target, mesh_atomic_get(source));
#endif
}
...
static inline void group_clear(atomic_t *target, atomic_t *source)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		mesh_atomic_and(&target[i], ~ mesh_atomic_get(&source[i]));
	}
#else
	mesh_atomic_and(target, ~ mesh_atomic_get(source));
#endif
}
...
static inline int group_popcount(atomic_t *target)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		count += popcount(mesh_atomic_get(&target[i]));
	}
#else
	return popcount(mesh_atomic_get(target));
#endif
}
