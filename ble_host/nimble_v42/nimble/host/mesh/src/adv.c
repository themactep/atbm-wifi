...
#define MESH_SCAN_INTERVAL_MS 100
#define MESH_SCAN_WINDOW_MS   100
...
static inline void adv_send(struct os_mbuf *buf)
{
	...
	//BT_INFO("Advertising started. Sleeping %u ms", duration);
...
static bool adv_flags_valid(struct os_mbuf *buf)
{
    u8_t flags = 0U;
...
static bool adv_service_uuid_valid(struct os_mbuf *buf, u16_t *uuid)
{
    ...
    *uuid = net_buf_simple_pull_le16(buf);
    ...
    if (*uuid == 0x1827 && //0X1827 for pb-gatt bearer
...
static void handle_adv_service_data(struct os_mbuf *buf,
                                    ...
                                    u16_t uuid, s8_t rssi)
{
    ...
    case 0x1827:
        if (bt_mesh_is_provisioner_en()) {
            if (buf->om_len != BLE_MESH_PROV_SRV_DATA_LEN) {
                BT_WARN("Invalid Mesh Prov Service Data length %,data:%s", buf->om_len,bt_hex(buf->om_data, buf->om_len));
                return;
            }
    ...
    case 0x1828:
        if (buf->om_len != BLE_MESH_PROXY_SRV_DATA_LEN1 &&
                buf->om_len != BLE_MESH_PROXY_SRV_DATA_LEN2) {
            BT_WARN("Invalid Mesh Proxy Service Data length %d", buf->om_len);
            return;
        }
			    ...
			    u8_t adv_type, struct os_mbuf *buf)
{
	...
	if (adv_type != BLE_HCI_ADV_TYPE_ADV_NONCONN_IND && adv_type != BLE_HCI_ADV_TYPE_ADV_IND) {
		return;
	}

#if BT_MESH_EXTENDED_DEBUG
	BT_INFO("len %u: %s", buf->om_len, bt_hex(buf->om_data, buf->om_len));
#endif
	...
	while (buf->om_len > 1) {
...
#if MYNEWT_VAL(BLE_MESH_PROVISIONER)
			if (bt_mesh_is_provisioner_en()) {
				provisioner_beacon_recv(buf,addr);
			}else
#endif
			{
		...
		case BLE_HS_ADV_TYPE_COMP_UUIDS16://3
			...
			break;
		case BLE_HS_ADV_TYPE_SVC_DATA_UUID16://pb-gatt func 16
...
#else
	xTaskCreate(mesh_adv_thread, "mesh_adv", 2048,
                NULL, MYNEWT_VAL(BLE_MESH_ADV_TASK_PRIO), &adv_task_tid);
#endif
...
int bt_mesh_scan_enable(void)
{
    ...
    rc = ble_gap_disc(g_mesh_addr_type, BLE_HS_FOREVER, &scan_param, provisioner_proxy_event, NULL);
    if(rc){
	...
	set_scan_bit();
#else
	rc = ble_gap_disc(g_mesh_addr_type, BLE_HS_FOREVER, &scan_param, NULL, NULL);
    if(rc){
        BT_INFO("ble_gap_disc ERR RC=%x",rc);
		return rc;
	}
