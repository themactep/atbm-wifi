...
void hb_send(struct bt_mesh_model *model)
{
		       ...
		       struct os_mbuf *buf)
{
	...
	BT_INFO("Friend Set 0x%02x -> 0x%02x", cfg->frnd, buf->om_data[0]);
			      ...
			      struct os_mbuf *buf)
{
	...
	if (sub_src == BT_MESH_ADDR_UNASSIGNED ||
		...
		//cfg->hb_sub.src = BT_MESH_ADDR_UNASSIGNED;//spec4.4.1.2.16 似乎要求为unassigned
		//cfg->hb_sub.dst = BT_MESH_ADDR_UNASSIGNED;
        period_ms = 0;
	}
	...
	if (period_ms) {
		cfg->hb_sub.expiry = k_uptime_get_32() + period_ms;
	} else {
		cfg->hb_sub.expiry = 0;
	}
