...
SRAM_CODE static struct ble_att_svr_entry *
 ...
 */
SRAM_CODE struct ble_att_svr_entry *
...
SRAM_CODE static struct ble_att_prep_entry *
...
{
    atbm_kfree(ble_att_svr_entry_mem);
...
ble_att_svr_start(void)
{
    ...
    if (ble_hs_max_attrs > 0) {
        ble_att_svr_entry_mem = atbm_kmalloc(
            OS_MEMPOOL_BYTES(ble_hs_max_attrs,
                             sizeof (struct ble_att_svr_entry)), GFP_KERNEL);
