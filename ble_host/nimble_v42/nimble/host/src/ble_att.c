...
SRAM_CODE int ble_att_conn_chan_find(uint16_t conn_handle, struct ble_hs_conn **out_conn,
...
SRAM_CODE void ble_att_inc_tx_stat(uint8_t att_op)
{
    switch (att_op) {
...
SRAM_CODE static void ble_att_inc_rx_stat(uint8_t att_op)
{
    switch (att_op) {
    case BLE_ATT_OP_ERROR_RSP:
...
SRAM_CODE void ble_att_truncate_to_mtu(const struct ble_l2cap_chan *att_chan,
                        struct os_mbuf *txom)
{
    int32_t extra_len;
    uint16_t mtu;
...
SRAM_CODE uint16_t ble_att_mtu(uint16_t conn_handle)
{
...
SRAM_CODE void ble_att_set_peer_mtu(struct ble_l2cap_chan *chan, uint16_t peer_mtu)
{
...
SRAM_CODE uint16_t ble_att_chan_mtu(const struct ble_l2cap_chan *chan)
{
    uint16_t mtu;
...
SRAM_CODE static void ble_att_rx_handle_unknown_request(uint8_t op, uint16_t conn_handle,
                                  struct os_mbuf **om)
{
...
SRAM_CODE static int ble_att_rx(struct ble_l2cap_chan *chan)
{
    const struct ble_att_rx_dispatch_entry *entry;
    uint8_t op;
...
SRAM_CODE uint16_t ble_att_preferred_mtu(void)
{
    return ble_att_preferred_mtu_val;
}

SRAM_CODE int ble_att_set_preferred_mtu(uint16_t mtu)
{
    struct ble_l2cap_chan *chan;
    struct ble_hs_conn *conn;
    int i;
...
SRAM_CODE struct ble_l2cap_chan *ble_att_create_chan(uint16_t conn_handle)
{
    struct ble_l2cap_chan *chan;
...
SRAM_CODE int ble_att_init(void)
{
    int rc;
