...
#if ((MYNEWT_VAL_BLE_HCI_TRANSPORT_UART) == 1)
#include "ble_uart.h"
#endif
struct ble_npl_eventq hci_uart_evq;
static TaskHandle_t hci_uart_task_h;
struct ble_npl_event ble_uart_rx_ev;
struct ble_npl_event ble_uart_tx_ev;
static int ble_hci_uart_rx_char(void *arg, uint8_t data);
static int ble_hci_uart_tx_char(void *arg);
static void ble_uart_rx_ev_func(struct ble_npl_event *ev);
static void ble_uart_tx_ev_func(struct ble_npl_event *ev);
uint8 ble_uart_rx_buf[300];
...
#if ((MYNEWT_VAL_BLE_HCI_TRANSPORT_UART) == 1)
u32 hci_read_buf_len;
u32 totol_rx_len;
u8 rx_buf[256];
int ble_uart_config(u32 port,u32 baudrate,u32 WordLength,u32 Stopbit,u32 parity,u32 HwFlowCtl){
	Console_SetPolling(1);
	iot_printf("port %x buad %x wdlgth %x stbit %x parity %x flow %x \n",port ,baudrate ,WordLength, Stopbit, parity ,HwFlowCtl);
	Uart_InitTpyeDef UART3;
	if(WordLength == 8){
	...
	UART3.uart_num = port;
	UART3.UART_IO_Mask = Mask_TX_IO0_RX_IO1;
	UART3.BaudRate = baudrate;
	UART3.WordLength = WordLength;
	UART3.Partity = parity;
	UART3.Stopbit = Stopbit;
	UART3.HwFlowCtl = HwFlowCtl;
	hal_uart_init(&UART3);
}
extern int hal_uart_recv_data(enum uart_num uartx,uint8 * pData);
void ble_uart_ReadDataInIrq(enum uart_num uartx)
{
    while(totol_rx_len < 256)
    {
        uint8 Data;
        if (!hal_uart_recv_data(uartx,&Data))
        {
            break;
        }
		if(Data != '\r' || Data != '\n'){
		    rx_buf[totol_rx_len] = Data;
			totol_rx_len++;
		}
		if(totol_rx_len == 4){
			hci_read_buf_len = totol_rx_len + Data;
		}
        if(totol_rx_len == hci_read_buf_len)
        {
        	totol_rx_len = 0;
			memcpy(ble_uart_rx_buf,rx_buf,hci_read_buf_len);
			int i;
      		for(i = 0;i<hci_read_buf_len;i++){
				iot_printf("%x ",ble_uart_rx_buf[i] );
			}
			iot_printf("\n");
			ble_npl_eventq_put(&hci_uart_evq, &ble_uart_rx_ev);
			hci_read_buf_len = 0;
			break;
        }
    }
}
void hal_uart_start_tx(u8 uartx)
{
	ble_npl_eventq_put(&hci_uart_evq, &ble_uart_tx_ev);
}
...
static void ble_uart_rx_ev_func(struct ble_npl_event *ev)
{
	...
	for(i = 0;i<len;i++){
		iot_printf("%x ",ble_uart_rx_buf[i] );
		ble_hci_uart_rx_char(NULL,ble_uart_rx_buf[i]);
	}
	iot_printf("\n");
}
static void ble_uart_tx_ev_func(struct ble_npl_event *ev)
 ...
 */
	...
	ble_npl_event_init(&ble_uart_rx_ev, ble_uart_rx_ev_func,NULL);
    ble_npl_event_init(&ble_uart_tx_ev, ble_uart_tx_ev_func,NULL);

	rc = ble_uart_config(MYNEWT_VAL(BLE_HCI_UART_PORT),
...
ble_hci_trans_reset(void)
{
    ...
    rc = ble_uart_close(MYNEWT_VAL(BLE_HCI_UART_PORT));
    if (rc != 0) {
    }
...
void ble_hci_uart_task(void *arg)
{
	struct ble_npl_event *ev;
	while (1) {
		ev = ble_npl_eventq_get(&hci_uart_evq, BLE_NPL_TIME_FOREVER);
		printf("ev->fn %x \n",ev->fn);
...
ble_hci_uart_init(void)
{
    ...
    xTaskCreate(ble_hci_uart_task, "hciuart", configMINIMAL_STACK_SIZE + 400,
                NULL, tskIDLE_PRIORITY + 1, &hci_uart_task_h);
}
