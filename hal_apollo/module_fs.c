...
struct atbm_stop_ba_parmas{
	struct atbm_conf_ifname ifname;
	struct atbm_conf_macs     macs;
	struct atbm_conf_integer type;
};
...
#define ATBM_CODE_STR__ATTR_STOP_BA_INFO			"stop_ba ifname=[xxxx] mac=[00:11:22:33:44:55] type=[x]"
#define ATBM_CODE_STR__ATTR_LMAC_DEBUG_INFO			"lmac_debug value=[1 or 0]"
...
#define ATBM_CODE_STR__ATTR_STOP_BA					"stop_ba"
#define ATBM_CODE_STR__ATTR_LMAC_DEBUG					"lmac_debug"
...
static bool atbm_module_attr_halt(struct atbm_common *hw_priv,struct atbm_store_cmd_code *cmd_store,
										const char *buf,int len);
static bool atbm_module_attr_stop_ba(struct atbm_common *hw_priv,struct atbm_store_cmd_code *cmd_store,
											const char *buf,int len);
static bool atbm_module_attr_lmac_debug(struct atbm_common *hw_priv,struct atbm_store_cmd_code *cmd_store,
											const char *buf,int len);
...
static bool atbm_module_attr_stop_ba(struct atbm_common *hw_priv,struct atbm_store_cmd_code *cmd_store,
											const char *buf,int len)
{
	#define STOP_IFNAME   "ifname="
	#define STOP_MAC  	  "mac="
	#define STOP_TYPE	  "type="
	struct atbm_stop_ba_parmas ba_parmas;
	struct atbm_sting_s stop_string[]={
		ATBM_STRING_TABLE_INIT(STOP_IFNAME,atbm_module_config_ifname,&ba_parmas.ifname),
		ATBM_STRING_TABLE_INIT(STOP_MAC,atbm_module_config_macs,&ba_parmas.macs),
	};
	const char* select_info = buf;
	struct ieee80211_sub_if_data *sdata = NULL;
	bool res = true;
	...
	if(atbm_string_parase(cmd_store,stop_string,ARRAY_SIZE(stop_string),select_info,len-(select_info-buf))==false){
		atbm_printk_always("%s:ap config %s err\n",__func__,select_info);
		res = false;
		goto exit;
	}

	if(ba_parmas.ifname.size == 0){
		atbm_printk_err("%s:ifname is null\n",__func__);
		res = false;
		goto exit;
	}

	sdata = atbm_module_get_sdata_by_name(ba_parmas.ifname.if_name);

	if(sdata == NULL){
		atbm_printk_err("%s:[%s] not found\n",__func__,ba_parmas.ifname.if_name);
		res = false;
		goto exit;
	}
	/*
	...
	*/
	...
	for(index = 0;index < ba_parmas.macs.n_macs;index++){
		sta = sta_info_get(sdata, ba_parmas.macs.mac[index].mac);
		if(sta){
...
exit:
	if(ba_parmas.macs.mac)
		atbm_kfree(ba_parmas.macs.mac);
	if(ba_parmas.type.integers)
		atbm_kfree(ba_parmas.type.integers);
	rtnl_unlock();
	return res;
}
static bool atbm_module_attr_lmac_debug(struct atbm_common *hw_priv,struct atbm_store_cmd_code *cmd_store,
...
{
	bool res = true;
	char *ccastart="ccastart 1";
	char *rate_p="set_rate_print 1";
	char ucDbgPrintOpenFlag = 0;
	int value = 0;

	sscanf(buf,"value=%d",&value);
	atbm_printk_debug("%s ,value = %d\n",buf,value);
	...
	if(value == 1){
		...
		wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,
				ccastart,11 , 0);
		wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,
				rate_p, 17 , 0);
	}else{
