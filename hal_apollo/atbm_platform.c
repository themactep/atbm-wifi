...
#define PLATFORMINF     "gk7202v330"
//É¨¿¨
extern int bsp_sdio_rescan(int slot);

#define GK7202V330_WIFI_RESET_GPIO  ((8*8) + 6)
#define POWER_REG (0x112C0050)

//power ctrl
static int ark_wifi_power_set(const struct atbm_platform_data *pdata,bool enabled)
{
#if 0
	himm(0x112C0050,0x1000);
	common_device_gpioOpen(GPIO_WIFI_POWER,0);
	usleep(1000*200);
	common_device_gpioWirte(GPIO_WIFI_POWER,0);
	usleep(1000*200);
	common_device_gpioWirte(GPIO_WIFI_POWER,1);
	usleep(1000*300);
#endif

	unsigned int *pRegVal = NULL;
	...
	*pRegVal = 0x1000;
	...
	if (gpio_request(pdata->power_gpio, "v330_wifi_reset")!= 0)
	{
	...
	printk(KERN_INFO"[v330] Success:set wifi_power %s\n", enabled? "on":"off");
...
//	if(enabled)
//	{
//		printk(KERN_INFO"[%s] reset at603 wifi\n", __func__);
//     	gpio_request(GK7202V330_WIFI_RESET_GPIO, "at603_wifi_power_on");
//		printk(KERN_INFO"[%s] reset at603 wifi reset 0\n", __func__);
//		gpio_direction_output(GK7202V330_WIFI_RESET_GPIO, 0);
//		msleep(300);
//		printk(KERN_INFO"[%s] reset at603 wifi reset 1\n", __func__);
//		gpio_direction_output(GK7202V330_WIFI_RESET_GPIO, 1);
//		msleep(100);
//	}
}
#endif
...
static int atbm_platform_insert_crtl(const struct atbm_platform_data *pdata,bool enabled)
{
...
#if (ATBM_WIFI_PLATFORM == PLATFORM_SUN8I)
	{
...
#if (ATBM_WIFI_PLATFORM == PLATFORM_GK7202V330)
	 {
		 ...
		 printk(KERN_INFO"[%s] reset at603 wifi rescan %d\n", __func__, enabled);
	 }
#endif
