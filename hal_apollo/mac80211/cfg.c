...
static void sta_set_sinfo(struct sta_info *sta, struct station_info *sinfo)
{
	...
	if ((sta->local->hw.flags & IEEE80211_HW_SIGNAL_DBM) ||
		...
		#endif
		sinfo->signal = (s8)-atbm_ewma_read(&sta->avg_signal);
...
static void ieee80211_config_ap_cap(struct ieee80211_sub_if_data *sdata,u8 * ie, size_t len)
{
	...
	*/
	if(sdata->vif.p2p == true){
		...
		ieee802_11_parse_elems(ie,len, &elems);
				 ...
				 enum nl80211_channel_type channel_type)
{
	...
	if(netdev){
		...
		sdata->vif.bss_conf.basic_rates = ieee80211_atbm_mandatory_rates(local,chan->band);
	}
				    ...
				    struct cfg80211_gtk_rekey_data *data)
{
	...
	ieee80211_rekey_set_data_enable(sdata,(u8 *)data->kck,(u8 *)data->kek);
	return 0;
}
#endif
#endif
