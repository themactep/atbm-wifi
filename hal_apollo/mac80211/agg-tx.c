...
static int ieee80211_tx_ba_alloc_hw_token(struct ieee80211_local *local)
{
	int token = -1;
	...
	token = find_first_zero_bit(local->tx_aggr_map,local->hw.max_hw_support_tx_aggs);

	if(token < 0 || token >= local->hw.max_hw_support_tx_aggs){
		token = -1;
		goto exit;
	}

	__set_bit(token,local->tx_aggr_map);
exit:
	spin_unlock_bh(&local->aggr_lock);
	return token;
}

static void ieee80211_tx_ba_free_hw_token(struct ieee80211_local *local,int token)
{
	spin_lock_bh(&local->aggr_lock);
	...
	__clear_bit(token,local->tx_aggr_map);
exit:
	spin_unlock_bh(&local->aggr_lock);
}

static int ieee80211_tx_ba_session_limit(struct sta_info *sta, int tid)
{
	return __ratelimit(&sta->ampdu_mlme.trylimit[tid]);
}
				    ...
				    bool tx)
{
	...
	ret = drv_ampdu_action(local, sta->sdata,
			       ...
			       &sta->sta, tid, NULL, 0, tid_tx->hw_token);
...
void ieee80211_tx_ba_session_handle_start(struct sta_info *sta, int tid)
{
	...
	ret = drv_ampdu_action(local, sdata, IEEE80211_AMPDU_TX_START,
			       &sta->sta, tid, &start_seq_num, 0, tid_tx->hw_token);
	if (ret) {
#ifdef CONFIG_MAC80211_ATBM_HT_DEBUG
...
#ifdef CONFIG_MAC80211_ATBM_HT_DEBUG
		atbm_printk_agg("BA request already send%d\n", tid);
#endif
		return;
	}
#ifdef CONFIG_MAC80211_ATBM_HT_DEBUG
...
//			   pubsta->addr);
	...
	if (ieee80211_tx_ba_session_limit(sta,tid) == 0) {
		ret = -EBUSY;
		goto err_unlock_sta;
	}
	tid_tx = rcu_dereference_protected_tid_tx(sta, tid);
	...
	if (tid_tx || sta->ampdu_mlme.tid_start_tx[tid]) {
		...
		atbm_printk_agg("BA request denied - session is not "
				 "idle on tid %u %p %p\n", tid,tid_tx,sta->ampdu_mlme.tid_start_tx[tid]);
#endif /* CONFIG_MAC80211_ATBM_HT_DEBUG */
		ret = -EAGAIN;
		goto err_unlock_sta;
	}
 ...
 err_unlock_sta:
 	if(ret != 0){
		ieee80211_tx_ba_free_hw_token(local,token);
 	}
	spin_unlock_bh(&sta->lock);
	return ret;
}
					 ...
					 struct sta_info *sta, u16 tid)
{
	...
	drv_ampdu_action(local, sta->sdata,
			 ...
			 &sta->sta, tid, NULL, tid_tx->buf_size, tid_tx->hw_token);
