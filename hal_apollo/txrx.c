...
int ieee80211_is_eapol_frame(struct sk_buff *skb)
{
	char *ip = NULL;
			...
			struct wsm_tx *wsm)
{
	...
	*/
...
#if defined (CONFIG_TX_NO_CONFIRM)
	...
	}else if(t->skb->protocol == cpu_to_be16(ETH_P_PAE)){
			wsm->htTxParameters |=__cpu_to_le32(WSM_NEED_TX_CONFIRM);
	...
	*/
	...
	if(wsm->maxTxRate >= 14){
		...
		*/
		if(tx_info->control.sta && ieee80211_is_data_qos(t->hdr->frame_control)){
#ifdef CONFIG_ATBM_DRIVER_PROCESS_BA
			/*
			*try to setup ba
			*/
			if(!ieee80211_hw_setup_ba(hw_priv->hw)){
				if(tx_info->flags & IEEE80211_TX_CTL_AMPDU){
					wsm->htTxParameters |= WSM_HT_AGGR_EN;
				}else if(ieee80211_start_tx_ba_session(tx_info->control.sta,t->txpriv.tid,0)){
					atbm_printk_limit("start tx ba session fail[%pM][%d]\n",&tx_info->control.sta->addr,t->txpriv.tid);
				}
			}
#endif
			/*
			*p2p not support 40M
			*4 way handshake force to 20M
			*/
			if ((priv->vif->p2p == false) && (ieee80211_chw_is_ht40(vif_chw(priv->vif))) &&
				(ieee80211_sta_support_40w(tx_info->control.sta))) {
		  ...
		  struct sk_buff **skb_p)
{
	...
	if(WSM_RX_STATUS_ENCRYPTION(arg->flags)){
		if(ieee80211_has_protected(frame->frame_control) || (arg->status == 0))
    ...
    if (priv->mode == NL80211_IFTYPE_AP &&( priv->join_status == ATBM_APOLLO_JOIN_STATUS_AP )&&
            ...
            ht_cap = atbm_ieee80211_find_ie(ATBM_WLAN_EID_HT_CAPABILITY,ies, ies_len);
            if(!ht_cap){
                    priv->ht_info |= 0x0011;
            }
